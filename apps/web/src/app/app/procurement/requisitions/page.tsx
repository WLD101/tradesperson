...
export default async function RequisitionsPage({
  ...
  const response = await apiFetch<{
    ...
    pageSize: number;
    totalPages: number;
  }>(`/api/v1/purchase-requisitions?${query.toString()}`);
    ...
    <div className="space-y-6">
      <Card>
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-slate-950">Purchase Requisitions</h1>
      ...
      <Card>
        <div className="flex items-center justify-between gap-4">
          <p className="text-sm text-slate-500">
            {response.total} requisitions across {response.totalPages} pages
          </p>
          <p className="text-sm text-slate-500">
                ...
                <tbody>
                  {response.items.map((item) => (
                    <tr key={item.id} className="rounded-xl bg-slate-50">
                      <td className="border-y border-slate-200 px-3 py-3 first:rounded-l-xl first:border-l">{item.requisitionNumber}</td>
                      ...
                      <td className="border-y border-slate-200 px-3 py-3 last:rounded-r-xl last:border-r">
                        <Link href={`/app/procurement/requisitions/${item.id}`} className="text-sm font-medium text-slate-700 hover:text-slate-950">View</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
